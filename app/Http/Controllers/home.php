...
class Home extends BaseController
{
    ...
    public function index(){
        ...
        $this->data["catchup_items"] = $api->getCatchupByDate(date("Y-m-d"), 6);
...
//        dd($this->data["latest"]);
    ...
    public function WeKnow(Request $request){
        //$time_start = $this->micro_time();

        $device_id = $request->input("device_id");
        $api = new ApiRequest();

        $this->data["most_viewed_categories"] ="";
        $this->data["most_viewed_shows"] ="";
        $this->data["device_id"]= $device_id;


        $data = $api->getWeKnowData($device_id);

        $this->data["most_viewed_categories"] = new \stdClass();
        $this->data["most_viewed_shows"] = new \stdClass();
        $this->data["videos_count"] = new \stdClass();
        $this->data["fav_channel"] = new \stdClass();

        if(isset($data->categories)) $this->data["most_viewed_categories"] = $data->categories;
        if(isset($data->shows)) $this->data["most_viewed_shows"] = $data->shows;
        if(isset($data->videos)) $this->data["videos_count"]->data = $data->videos;
        if(isset($data->channels)) $this->data["fav_channel"]->data = $data->channels;

        //dd($data);
        //dd($this->data);

//        $this->data["most_viewed_categories"] = $api->getMostViewedCategories($device_id);
//        $this->data["most_viewed_shows"] = $api->getMostViewedShows($device_id);
//        $this->data["videos_count"] = $api->getVideosViews($device_id);
//        $this->data["fav_channel"] = $api->getFavChannel($device_id);

        $this->data["cdn"] =config('mangoapi.mangodcn');
        ...
        if(isset($videos_count) && isset($videos_count->data->count) && count($videos_count) > 0){
            $text = $this->data["videos_count"]->data->count;
            //$img->createImage($text);
            //save image as png format
            //$img->saveAsJpg('test_' . $t . "_" . $device_id,public_path('images/'));
            //$img->showImage();
        }

        //$time_stop = $this->micro_time();
        //$time_overall = bcsub($time_stop, $time_start, 6);
        //echo "Execution time - $time_overall Seconds";

        return $this->view('weknow');
    }

    function shortenURL(Request $request){

        $query = http_build_query($request->input());
        $c = file_get_contents("http://api.bit.ly/v3/shorten?$query");
...
//        $page_key = md5('awaanvideos1');
//        $content = \Cache::get($page_key);
//        if (!\Cache::has($page_key)) {
            ...
            $content = $tidy->repairString($content,$tidyConfig);
...
//            ->header('Content-Transfer-Encoding', 'binary')
    ...
    function mrss_video($video_id){

        $api = new ApiRequest();
        $video_playback = $api->getPlaybackURL($video_id);
        $video = $api->getVideoDetails($video_id);

        if(isset($video_playback->URL)){
            $video->playback = $video_playback->URL;
        }
